...
class ContentController {
    ...
    fun getContentById (@PathVariable id : Int) : ResponseEntity <ContentDto> {
        return try {
    ...
    fun add (@RequestBody content : ContentRequest) : ResponseEntity <Void> {
        return try {
    ...
    fun filter (@RequestParam name : Optional<String>, @RequestParam genre : Optional<String>,
        ...
        return try {
            ...
            var itemsPerPage : Int = items_per_page.orElseGet{ 3 }
            if (itemsPerPage < 0)
                itemsPerPage = 3

            val request = FilterRequest(name1, genre1, year1, match1)

            val listFiltered : List<Content> = contentService.filter(request)
            val response = ContentList(contentService.pagination(listFiltered, itemsPerPage, page1))
            ResponseEntity (response, HttpStatus.OK)
        } catch (exception : Exception) {
            ResponseEntity(HttpStatus.INTERNAL_SERVER_ERROR)
        }
